...
function extractHints(text) {
    ...
    const directTemplateMatch = normalized.match(/(\d+)\s*(?:-|\s*)?(cone|line|cube|cylinder|sphere)\b/i);
...
function ensureAutomationFlagContainer(item) {
    ...
    item.flags["vibe-actor"] = item.flags["vibe-actor"] || {};
    if (!Array.isArray(item.flags["vibe-actor"].automationWarnings)) {
        item.flags["vibe-actor"].automationWarnings = [];
    }
}
...
export async function ensureItemHasImage(item) {
    if (item.img && item.img.startsWith("icons/svg/")) return;
...
export function normalizeMutuallyExclusiveOptionItems(items) {
    ...
    for (const baseIndex of candidateBaseIndexes.values()) {
        ...
        baseItem.flags["vibe-actor"].normalizedOptionStructure = true;
    }
...
export function validateAndRepairItemAutomation(item) {
    ...
    if (warnings.length > 0) {
        cloned.flags["vibe-actor"].automationWarnings = warnings;
        console.warn("Vibe Actor | Automation warnings:", warnings);
    } else {
        cloned.flags["vibe-actor"].automationWarnings = [];
    }
