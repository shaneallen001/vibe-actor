...
export async function createContainerFeats(actor, multiattackText, spellcastingText) {
    ...
    if (!actor || !actor.items) {
        console.warn("Vibe Actor | createContainerFeats called with invalid actor:", actor);
        return;
    }
    ...
    if (itemsToCreate.length > 0) {
        ...
        console.log(`Vibe Actor | Added ${itemsToCreate.length} container feats to ${actor.name}.`);
    }
}
